	...
	public void convertToBaf(final JimpleToBafContext context, final List<Unit> out) {
		...
		lvalue.apply(new AbstractJimpleValueSwitch() {
			...
			public void caseLocal(final Local v) {
				...
				/*
				 * Add the tags to the statement that COMPUTES the value, NOT to
				 * the statement that stores it.
				 */

				/*
				 * No: the convertToBaf on the rvalue already adds them, so no
				 * need to add them here. However, with the current semantics,
				 * we should add them to every statement and let the aggregator
				 * sort them out.
				 */
